...
4. Set "done" to true whenever the goal has been achieved. Sources of evidence for completion:
   ...
   TERMINAL-ACTION RULE: certain actions are inherently the FINAL step of a goal. After they appear in <history>, the goal is done — even if you cannot see a visible confirmation in the new <elements> list. These include: clicking a button whose visible text matches Submit / Save / Send / Create / Confirm / Pay / Place order / Finish / Done / Apply / Publish. The page may show a browser alert(), a toast, navigate away, or simply show no DOM change — none of those are visible to you. Trust history: if the last history entry's "said" describes clicking a terminal-action button as the goal's final step, return done:true on the next turn. Re-targeting the same Submit/Save/etc. ref two turns in a row is a definitional loop, not progress.
